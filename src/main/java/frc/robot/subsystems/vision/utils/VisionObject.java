...
public class VisionObject {
    static NetworkTable table = NetworkTableInstance.getDefault().getTable(RobotMap.VisionMap.BackCam.BACK_CAMERA_NETWORK_TABLES_NAME);
    private double x;
    private double y;
    private double area;
    ...
    public double[] getAngle() {
        switch (type) {
            case NOTE:
                ...
                double nx = ((double) 1 / ((double) RobotMap.VisionMap.FrontCam.CameraWidth / 2)) * (x - ((double) RobotMap.VisionMap.FrontCam.CameraWidth / 2 - 0.5));
                double ny = ((double) 1 / ((double) RobotMap.VisionMap.FrontCam.CameraHeight / 2)) * (y - ((double) RobotMap.VisionMap.FrontCam.CameraHeight / 2 - 0.5));
                double vpw = 2.0 * Math.tan(RobotMap.VisionMap.FrontCam.horizontal_fov / 2);
                double vph = 2.0 * Math.tan(RobotMap.VisionMap.FrontCam.vertical_fov / 2);
                double x = vpw / 2 * nx;
                double y = vph / 2 * ny;
            ...
            case APRILTAG:
                ...
                double nx1 = ((double) 1 / ((double) RobotMap.VisionMap.BackCam.CameraWidth / 2)) * (this.x - ((double) RobotMap.VisionMap.BackCam.CameraWidth / 2 - 0.5));
                double ny1 = ((double) 1 / ((double) RobotMap.VisionMap.BackCam.CameraHeight / 2)) * (this.y - ((double) RobotMap.VisionMap.BackCam.CameraHeight / 2 - 0.5));
                double vpw1 = 2.0 * Math.tan(RobotMap.VisionMap.BackCam.horizontal_fov / 2);
                double vph1 = 2.0 * Math.tan(RobotMap.VisionMap.BackCam.vertical_fov / 2);
    ...
    public Pose3d getBotPose(){
        return LimelightHelpers.getBotPose3d(RobotMap.VisionMap.BackCam.BACK_CAMERA_NETWORK_TABLES_NAME);
    }
